package com.hanyi._02static关键字;

/**
 * 目标：static关键字的概述。（emphasis）
 * 引入：
 *      我们之前定义了很多成员变量（name，age，sex）
 *      其实我们只写了一份，但是发现每个对象都可以用，就说明
 *      Java中这些成员变量或这个方法是存在所属性的。
 *      有些是属于对象的，有些是数据类本身的。
 ...
 *      成员变量：
 *          （1）静态成员变量（类变量）：
 *                  有static修饰的成员变量称为静态变量也叫类变量，属于类本身的，直接用类名访问即可。
 *          （2）实例成员变量
 ...
 *          -- 无static修饰的属于类的每个对象的叫实例成员变量，与类的对象一起加载，对象有多少个，
 *              实例成员变量就加载多少份。必须用类的对象调用
 *      成员方法有2种
 *          -- 有static修饰的属于类叫静态方法，直接用类名调用即可。
 *          -- 无static修饰的属于类的每个对象的实例方法，必须用类的对象调用。
 */
