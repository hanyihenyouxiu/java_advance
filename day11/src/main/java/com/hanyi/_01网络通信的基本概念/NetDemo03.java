 ...
 * 目标：网络通信的分层和协议。
 *
 * 网络通信协议：通信协议是对计算机必须遵循的规则。
 * 只有遵守这些规则，计算机之间才能进行通信。
 *
 * --------------------------------------------------------------------------------
 * 应用层：应用程序(QQ、微信、浏览器)，可能用到的协议(HTTP、FTP、SMTP) 通常程序员值需要关心这一层。
 * --------------------------------------------------------------------------------
 * 传输层：TCP/IP协议---UPD协议     计算机网络工程师需要精通的协议，有些技术我们也需要精通这一层协议。
 * --------------------------------------------------------------------------------
 ...
 *  2.例如下载文件，浏览网页等（需要可靠传输）
 *
 *  UPD：用户数据报协议（User Datagram Protocol）
 *      UDP协议是一个面向无连接的一个不可靠的协议。
