package com.hanyi._02Java常见数据结构种类;

/**
 * 目标：常见的数据结构种类
 *
 * 集合是基本数据结构做出来的，不同的集合底层会采用不同的数据结构。
 * 不同的数据结构，功能和作用是不一样的。
 *
 * 什么是数据结构？
 *  数据机构指的是数据以什么方式组织在一起。
 *  不同的数据结构，增删查的性能是不一样的。
 *  不同的集合底层会采用不同的数据结构，我们要知道集合的底层是基于数据结构存储和操作数据的。
 *      这样才能知道具体场景用那种集合。
 *
 *  Java常见的数据结构有哪些？
