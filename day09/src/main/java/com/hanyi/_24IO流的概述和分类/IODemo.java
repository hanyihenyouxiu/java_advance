package com.hanyi._24IO流的概述和分类;
 ...
 *      (2)输入流：以内存为基准，把磁盘文件中的数据或者网络介质中的数据读入到内存中去的流称为输入流。
 *          输入流的作用：读取数据到内存。
 *  按照流的内容分为：字节流，字符流
 ...
 *  字符输入流：以内存为基准，把磁盘文件中的数据或者网络中的数据以一个一个的字符的形式读入到内存中去的流称为字符输出流。
 *  字符输出流：以内存为基准，把内存中的数据以一个一个字符写出到磁盘文件或者网络介质中去的流称为字符输输出流。
 *
 * 小结：
